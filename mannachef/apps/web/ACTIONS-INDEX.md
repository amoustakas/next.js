...
## `server/actions/referral-claim.ts`

| export                     | action                   | auth    | input schema                  | returns                       | rate-limited |
| -------------------------- | ------------------------ | ------- | ----------------------------- | ----------------------------- | ------------ |
| `readPendingReferralClaim` | `referral.claim.read`    | SESSION | `emptyInputSchema`            | `PendingReferralClaimView     | null`        | yes |
| `acceptReferralClaim`      | `referral.claim.accept`  | SESSION | `referralClaimDecisionSchema` | `ReferralClaimAcceptanceView` | yes          |
| `declineReferralClaim`     | `referral.claim.decline` | SESSION | `referralClaimDecisionSchema` | `ReferralClaimDeclineView`    | yes          |
